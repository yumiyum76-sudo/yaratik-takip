...
function getQuestCities() {
  return ['Britain', 'Moonglow', 'Trinsic', 'Vesper', 'Yew', 'Jhelom', 'Minoc', 'Skara', 'Occlo', 'Bucs'];
}
function getDefaultQuests() {
  return getQuestCities()
    .map(city => ({ city, startedBy: null, startedUsername: null, startedAt: null, result: '', resultBy: null, resultUsername: null }));
}
function getDefaultQuestReservations() {
  return getQuestCities()
    .map(city => ({ city, reservedBy: null, reservedUsername: null, reservedAt: null }));
}
...
function getNextQuestWeekStart(nowDate = new Date()) {
  const date = new Date(`${getQuestWeekStart(nowDate)}T00:00:00`);
...
function resetWeeklyQuestReservations(data, nowDate = new Date()) {
  const week = getNextQuestWeekStart(nowDate);
  if (data.reservationWeekStart !== week) {
    data.reservationWeekStart = week;
    data.questReservations = getDefaultQuestReservations();
  }
  return data.questReservations;
}
function createInitialData() {
  return {
    users: [{ name: 'Göksel', username: 'goksel', password: '1234', isAdmin: true }],
    creatures: getDefaultCreatures().map(item => ({ ...item, lastTime: null, killedBy: null, killedUsername: null })),
    deletedNames: [],
    questWeekStart: getQuestWeekStart(),
    quests: getDefaultQuests(),
    reservationWeekStart: getNextQuestWeekStart(),
    questReservations: getDefaultQuestReservations()
  };
}
function ensureDataShape(data) {
  ...
  const users = Array.isArray(data.users) ? data.users.map(user => ({
    ...user,
    isAdmin: cleanText(user.username) === 'goksel' ? true : !!user.isAdmin
  })) : initial.users;
  ...
  const ordered = getDefaultCreatures()
    ...
    .map(base => ({
      ...
      killedUsername: byName.get(base.name)?.killedUsername || null
    }));
  ...
  const reservationWeekStart = data.reservationWeekStart || getNextQuestWeekStart();
  const savedReservations = Array.isArray(data.questReservations) ? data.questReservations : [];
  const reservationByCity = new Map(savedReservations.map(item => [item.city, item]));
  const questReservations = getDefaultQuestReservations().map(base => ({ ...base, ...(reservationByCity.get(base.city) || {}) }));
  const shaped = { users, creatures: ordered, deletedNames, questWeekStart, quests, reservationWeekStart, questReservations };
  resetWeeklyQuests(shaped);
  resetWeeklyQuestReservations(shaped);
  return shaped;
}
function loadData() {
...
function canResetQuests(user) {
  return !!user && (cleanText(user.username) === 'goksel' || user.isAdmin === true);
}
function addUser(data, user, currentUser) {
  ...
  data.users.push({ name, username, password, isAdmin: !!user.isAdmin });
...
function updateUser(data, username, changes, currentUser) {
  ...
  user.isAdmin = cleanText(user.username) === 'goksel' ? true : changes.isAdmin === true;
...
function setCreatureTime(data, index, value, currentUser, nowDate = new Date()) {
  ...
  data.creatures[index].killedUsername = parsed ? currentUser.username : null;
  return data.creatures[index];
}
function isSameUser(savedName, savedUsername, currentUser) {
  if (!currentUser) return false;
  if (savedUsername) return cleanText(savedUsername) === cleanText(currentUser.username);
  return cleanText(savedName) === cleanText(currentUser.name);
}
function cancelCreatureTime(data, index, currentUser) {
  const creature = data.creatures[index];
  if (!creature) throw new Error('Bulunamadı');
  if (!creature.lastTime) throw new Error('İptal edilecek kayıt yok');
  if (!isSameUser(creature.killedBy, creature.killedUsername, currentUser)) throw new Error('Sadece işleyen kişi iptal edebilir');
  creature.lastTime = null;
  creature.killedBy = null;
  creature.killedUsername = null;
  return creature;
}
...
function setQuestResult(data, city, result, currentUser) {
  ...
  quest.resultUsername = value ? currentUser.username : null;
  return quest;
}
function cancelQuest(data, city, currentUser) {
  const quest = findQuest(data, city);
  if (!quest.startedBy) throw new Error('İptal edilecek kayıt yok');
  if (!isSameUser(quest.startedBy, quest.startedUsername, currentUser)) throw new Error('Sadece başlayan kişi iptal edebilir');
  quest.startedBy = null;
  quest.startedUsername = null;
  quest.startedAt = null;
  quest.result = '';
  quest.resultBy = null;
  quest.resultUsername = null;
  return quest;
}
function findQuestReservation(data, city) {
  resetWeeklyQuestReservations(data);
  const cleanCity = String(city || '').trim().toLocaleLowerCase('tr-TR');
  const reservation = data.questReservations.find(item => item.city.toLocaleLowerCase('tr-TR') === cleanCity);
  if (!reservation) throw new Error('Şehir bulunamadı');
  return reservation;
}
function reserveQuest(data, city, currentUser, nowDate = new Date()) {
  resetWeeklyQuestReservations(data, nowDate);
  const reservation = findQuestReservation(data, city);
  reservation.reservedBy = currentUser.name;
  reservation.reservedUsername = currentUser.username;
  reservation.reservedAt = nowDate.toISOString();
  return reservation;
}
function cancelQuestReservation(data, city, currentUser) {
  const reservation = findQuestReservation(data, city);
  if (!reservation.reservedBy) throw new Error('İptal edilecek kayıt yok');
  if (!isSameUser(reservation.reservedBy, reservation.reservedUsername, currentUser)) throw new Error('Sadece yazan kişi iptal edebilir');
  reservation.reservedBy = null;
  reservation.reservedUsername = null;
  reservation.reservedAt = null;
  return reservation;
}
function resetQuestsManually(data, currentUser) {
  if (!canResetQuests(currentUser)) throw new Error('Yetki yok');
  data.questWeekStart = getQuestWeekStart();
  data.quests = getDefaultQuests();
  return data.quests;
}
...
function publicUser(user) {
  return user ? {
    ...
    isAdmin: canCreateAccount(user),
    canManageUsers: canCreateAccount(user),
    canResetQuests: canResetQuests(user)
  } : null;
}
...
async function handleApi(req, res) {
  ...
  try {
    if (req.method === 'POST' && url.pathname === '/api/login') {
      ...
      sessions.set(token, { name: found.name, username: found.username, isAdmin: !!found.isAdmin, lastSeen: Date.now() });
    ...
    if (req.method === 'GET' && url.pathname === '/api/state') {
      return sendJson(res, 200, { user: publicUser(user), users: data.users.map(publicUser), creatures: data.creatures, quests: data.quests, questReservations: data.questReservations, reservationWeekStart: data.reservationWeekStart });
    }
    ...
    if (req.method === 'POST' && url.pathname === '/api/creatures') {
      ...
      data.creatures.push({ name, group: getGroupForName(name), lastTime: null, killedBy: null, killedUsername: null });
      saveData(data);
    ...
    if (req.method === 'POST' && markMatch) {
      ...
      data.creatures[index].killedUsername = user.username;
      saveData(data);
      return sendJson(res, 200, { creature: data.creatures[index] });
    }
    const cancelCreatureMatch = url.pathname.match(/^\/api\/creatures\/(\d+)\/cancel$/);
    if (req.method === 'POST' && cancelCreatureMatch) {
      const index = Number(cancelCreatureMatch[1]);
      const creature = cancelCreatureTime(data, index, user);
      saveData(data);
      return sendJson(res, 200, { creature, creatures: data.creatures });
    }
    ...
    if (req.method === 'PUT' && userUpdateMatch) {
      ...
      for (const sessionUser of sessions.values()) {
        if (cleanText(sessionUser.username) === cleanText(oldUsername)) {
          ...
          sessionUser.isAdmin = canResetQuests(updated);
        }
      }
      saveData(data);
    ...
    const questCancelMatch = url.pathname.match(/^\/api\/quests\/([^/]+)\/cancel$/);
    if (req.method === 'POST' && questCancelMatch) {
      const quest = cancelQuest(data, decodeURIComponent(questCancelMatch[1]), user);
      saveData(data);
      return sendJson(res, 200, { quest, quests: data.quests });
    }
    const reservationStartMatch = url.pathname.match(/^\/api\/quest-reservations\/([^/]+)\/reserve$/);
    if (req.method === 'POST' && reservationStartMatch) {
      const reservation = reserveQuest(data, decodeURIComponent(reservationStartMatch[1]), user);
      saveData(data);
      return sendJson(res, 200, { reservation, questReservations: data.questReservations });
    }
    const reservationCancelMatch = url.pathname.match(/^\/api\/quest-reservations\/([^/]+)\/cancel$/);
    if (req.method === 'POST' && reservationCancelMatch) {
      const reservation = cancelQuestReservation(data, decodeURIComponent(reservationCancelMatch[1]), user);
      saveData(data);
      return sendJson(res, 200, { reservation, questReservations: data.questReservations });
    }
    if (req.method === 'POST' && url.pathname === '/api/quests/reset') {
      const quests = resetQuestsManually(data, user);
      saveData(data);
...
module.exports = { createInitialData, getDefaultCreatures, getStatus, addUser, canCreateAccount, canResetQuests, deleteCreature, setCreatureTime, cancelCreatureTime, deleteUser, updateUser, getOnlineUsers, getDefaultQuests, startQuest, setQuestResult, cancelQuest, resetWeeklyQuests, resetQuestsManually, getDefaultQuestReservations, reserveQuest, cancelQuestReservation, resetWeeklyQuestReservations, startServer };
