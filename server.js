...
function ensureDataShape(data) {
  ...
  return { users, creatures: ordered, deletedNames };
}
function loadData() {
...
function setCreatureTime(data, index, value, currentUser) {
  if (!data.creatures[index]) throw new Error('Bulunamadı');
  data.creatures[index].lastTime = value ? new Date(value).toISOString() : null;
  data.creatures[index].killedBy = value ? currentUser.name : null;
...
module.exports = { createInitialData, getDefaultCreatures, getStatus, addUser, canCreateAccount, deleteCreature, setCreatureTime, deleteUser, startServer };
