...
function renderAccounts() {
  ...
  users.forEach(user => {
    ...
    item.textContent = `${user.name} — ${user.username}${user.isAdmin ? ' — Yetkili' : ''}`;
...
function render() {
  ...
  creatures.forEach((creature, index) => {
    ...
    item.innerHTML = `<div class="dot"></div><div class="info"><h3>${creature.name} — ${status.label}</h3><p>${status.detail}</p><p>${killedBy}</p><input type="datetime-local" value="${lastValue}" aria-label="Zaman seç" /></div><div class="actions"><button class="kill">Kesildi</button></div>`;
