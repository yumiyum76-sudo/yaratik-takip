...
let creatures = [];
let quests = [];
let questReservations = [];
let reservationWeekStart = '';
...
const resetQuestBtnEl = document.getElementById('resetQuestBtn');
const reservationTabEl = document.getElementById('reservationTab');
const reservationPageEl = document.getElementById('reservationPage');
const reservationListEl = document.getElementById('reservationList');
const reservationWeekEl = document.getElementById('reservationWeek');
...
function showApp() {
  ...
  accountCardEl.classList.toggle('hidden', !currentUser.canManageUsers);
  usersTabEl.classList.toggle('hidden', !currentUser.canManageUsers);
  onlineTabEl.classList.toggle('hidden', !currentUser.canManageUsers);
  resetQuestBtnEl?.classList.toggle('hidden', !currentUser.canResetQuests);
  renderAccounts();
  render();
  renderQuests();
  renderReservations();
...
function setPage(page) {
  ...
  reservationPageEl.classList.toggle('hidden', page !== 'reservation');
  ...
  reservationTabEl.classList.toggle('active', page === 'reservation');
...
async function loadState() {
  ...
  quests = data.quests || [];
  questReservations = data.questReservations || [];
  reservationWeekStart = data.reservationWeekStart || '';
  showApp();
}
...
async function createAccount() {
  try {
    ...
    const adminInput = document.getElementById('newAccountAdmin');
    const data = await request('/api/users', { method: 'POST', body: { name: nameInput.value, username: usernameInput.value, password: passwordInput.value, isAdmin: adminInput.checked } });
    users = data.users;
    nameInput.value = '';
    usernameInput.value = '';
    passwordInput.value = '';
...
async function cancelCreature(index) {
  await request(`/api/creatures/${index}/cancel`, { method: 'POST' });
  await loadState();
}
...
async function updateUserItem(oldUsername, item) {
  ...
  const isAdmin = item.querySelector('.edit-admin')?.checked || false;
  const data = await request(`/api/users/${encodeURIComponent(oldUsername)}`, { method: 'PUT', body: { name, username, password, isAdmin } });
  users = data.users;
...
function renderAccounts() {
  ...
  users.forEach(user => {
    ...
    item.innerHTML = `<input class="edit-name" value="${user.name}" placeholder="İsim" />
      ...
      <label class="check-row"><input class="edit-admin" type="checkbox" ${user.canResetQuests ? 'checked' : ''} ${user.username === 'goksel' ? 'disabled' : ''} /> Yetkili</label>
...
async function cancelQuestItem(city) {
  const data = await request(`/api/quests/${encodeURIComponent(city)}/cancel`, { method: 'POST' });
  quests = data.quests;
  renderQuests();
}
...
async function resetQuests() {
  if (!currentUser?.canResetQuests) return;
  if (!confirm('Binek Quest sıfırlansın mı?')) return;
  const data = await request('/api/quests/reset', { method: 'POST' });
  quests = data.quests;
  renderQuests();
}
function renderQuests() {
  ...
  quests.forEach(quest => {
    ...
    const canCancelQuest = quest.startedBy && (quest.startedUsername ? quest.startedUsername === currentUser?.username : quest.startedBy === currentUser?.name);
    item.innerHTML = `<div><h3>${quest.city}</h3><p>${started}</p><p>Sonuç: ${result || '-'}</p></div>
      <div class="quest-actions">
        <button class="start-quest">Queste başladım</button>
        ${canCancelQuest ? '<button class="cancel-quest danger">İptal</button>' : ''}
    ...
    item.querySelector('.cancel-quest')?.addEventListener('click', () => cancelQuestItem(quest.city));
...
async function reserveQuestItem(city) {
  const data = await request(`/api/quest-reservations/${encodeURIComponent(city)}/reserve`, { method: 'POST' });
  questReservations = data.questReservations;
  renderReservations();
}
async function cancelReservationItem(city) {
  const data = await request(`/api/quest-reservations/${encodeURIComponent(city)}/cancel`, { method: 'POST' });
  questReservations = data.questReservations;
  renderReservations();
}
function renderReservations() {
  if (!reservationListEl) return;
  reservationWeekEl.textContent = reservationWeekStart ? `Hafta: ${formatDateText(reservationWeekStart)}` : '';
  reservationListEl.innerHTML = '';
  questReservations.forEach(reservation => {
    const item = document.createElement('div');
    item.className = 'quest-row';
    const reserved = reservation.reservedBy ? `Başlayacak: ${reservation.reservedBy}` : 'Başlayacak: -';
    const canCancelReservation = reservation.reservedBy && (reservation.reservedUsername ? reservation.reservedUsername === currentUser?.username : reservation.reservedBy === currentUser?.name);
    item.innerHTML = `<div><h3>${reservation.city}</h3><p>${reserved}</p></div>
      <div class="quest-actions">
        <button class="reserve-quest">Buna başlayacağım</button>
        ${canCancelReservation ? '<button class="cancel-reservation danger">İptal</button>' : ''}
      </div>`;
    item.querySelector('.reserve-quest').addEventListener('click', () => reserveQuestItem(reservation.city));
    item.querySelector('.cancel-reservation')?.addEventListener('click', () => cancelReservationItem(reservation.city));
    reservationListEl.appendChild(item);
  });
}
function render() {
  listEl.innerHTML = '';
  ...
  creatures.forEach((creature, index) => {
    ...
    const canCancelCreature = creature.lastTime && (creature.killedUsername ? creature.killedUsername === currentUser?.username : creature.killedBy === currentUser?.name);
    item.innerHTML = `<div class="dot"></div><div class="info"><h3>${creature.name} — ${status.label}</h3><p>${status.detail}</p><p>${killedBy}</p><input type="text" inputmode="numeric" pattern="[0-9:]*" placeholder="15 veya 15:30" value="${lastValue}" aria-label="Saat yaz" /></div><div class="actions"><button class="kill">Kesildi</button>${canCancelCreature ? '<button class="cancel danger">İptal</button>' : ''}${currentUser?.canManageUsers ? '<button class="delete danger">Sil</button>' : ''}</div>`;
    item.querySelector('.kill').addEventListener('click', () => markKilled(index));
    item.querySelector('.cancel')?.addEventListener('click', () => cancelCreature(index));
...
reservationTabEl.addEventListener('click', () => setPage('reservation'));
resetQuestBtnEl?.addEventListener('click', resetQuests);
