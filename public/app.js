...
function render() {
  ...
  creatures.forEach((creature, index) => {
    ...
    item.innerHTML = `<div class="dot"></div><div class="info"><h3>${creature.name} — ${status.label}</h3><p>${status.detail}</p><p>${killedBy}</p><input type="time" value="${lastValue}" aria-label="Saat seç" /></div><div class="actions"><button class="kill">Kesildi</button>${currentUser?.isAdmin ? '<button class="delete danger">Sil</button>' : ''}</div>`;
